...
class AccountAnalyticLine(models.Model):
    ...
    def default_get(self, field_list):
        ...
        if res.get('employee_id'):
            res.pop('employee_id', None)
        return res

    timesheet_line_user_id = fields.Many2one(
        'res.users', "Current timesheet user",
        default=lambda self: self.env.user)

    @api.onchange('timesheet_line_user_id')
    def onchange_employee_id_line(self):
        domain = {'employee_id': []}
        hr_emp_obj = self.env['hr.employee']
        employee_id = hr_emp_obj.search(
            [('user_id', '=', self.timesheet_line_user_id.id)], limit=1)

...
class TestInvoice(TestCommonMixin):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.selected_lines = cls.lines.filtered(lambda l: l.date <= cls.date_08)
    ...
    def test_invoice_delivered_limited(self):
        ...
        if self.so_line.qty_delivered:
            invoice = self.get_invoice()
            invoice.action_post()
            self.assertEqual(len(invoice.invoice_line_ids), 1)
            self.assertEqual(sum(invoice.invoice_line_ids.mapped("quantity")), 1)
            self.assertEqual(self.selected_lines.timesheet_invoice_id, invoice)
    ...
    def test_invoice_multiple_invoices(self):
        ...
        if self.so_line.qty_delivered:
            ...
            self.assertEqual(sum(first_invoice.invoice_line_ids.mapped("quantity")), 1)
            self.assertEqual(self.selected_lines.timesheet_invoice_id, first_invoice)
            self.assertEqual(len(second_invoice.invoice_line_ids), 1)
            self.assertEqual(sum(second_invoice.invoice_line_ids.mapped("quantity")), 2)
            self.assertEqual(

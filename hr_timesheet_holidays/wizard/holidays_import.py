...
class HolidaysImport(osv.osv_memory):
    ...
    def _get_default_holidays(self, cr, uid, context=None):
        ...
        (((date_from <= %s and date_to >= %s and date_to <= %s) or\
          ...
          (date_from >= %s and date_from <= %s and date_to >= %s and date_to"
                   " <= %s) or\
          (date_from <= %s and date_to >= %s)) and user_id = %s and state ="
                   " 'validate')",
        ...
        if not holidays:
            raise osv.except_osv(_('Information'),
                                 _('No holidays for the current timesheet.'))
        ...
        for holiday in holidays:
            ...
            for day in range(nb_days):
                str_datetime_current = (
                    datetime.strptime(h_date_from, '%Y-%m-%d %H:%M:%S')
    ...
    _columns = {
        'holidays_ids': fields.many2many('hr.holidays', 'hr_holidays_rel',
                                         'wid', 'hid', 'Holidays',
    ...
    def import_holidays(self, cr, uid, ids, context):
        ...
        if not hours_per_day:
            raise osv.except_osv(_('UserError'), _(
                'The number of hours per day is not configured '
                'on the company.'))
        ...
        for holiday in wizard.holidays_ids:
            if not holiday.holiday_status_id.analytic_account_id.id:
                raise osv.except_osv(_('Error !'), _(
                    "Holiday Leave Type %s has no "
                    "associated analytic account !") % (
            ...
            for day in range(nb_days):
                dt_current = (
                    datetime.strptime(holiday.date_from, '%Y-%m-%d %H:%M:%S')
                    + timedelta(days=day))
                ...
                if not existing_attendances:
                    ...
                    date_end = dt_current.replace(hour=hours[0],
                                                  minute=hours[1])

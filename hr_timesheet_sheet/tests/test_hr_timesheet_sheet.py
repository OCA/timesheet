...
class TestHrTimesheetSheet(TransactionCase):
    ...
    def test_15(self):
        ...
        # breaks consistency check on analytic line
        with self.assertRaises(UserError):
    ...
    def test_16(self):
        ...
        # the form forbids writing on an invisible field,
        # so we create it manually instead to test the onchange
        vals = {
            "employee_id": new_employee.id,
            "department_id": False,
            "company_id": self.company.id,
        }
        model = self.sheet_model.with_user(self.user)
        sheet_no_department = model.create(vals)

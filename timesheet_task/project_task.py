...
class ProjectTask(osv.osv):
    ...
    def _progress_rate(self, cr, uid, ids, names, arg, context=None):
        ...
        for task in self.browse(cr, uid, ids, context=context):
            ...
            res[task.id]['total_hours'] = (
                task.remaining_hours or 0.0) + hours.get(
                task.id, 0.0)
            res[task.id]['delay_hours'] = \
                res[task.id]['total_hours'] - task.planned_hours
            res[task.id]['progress'] = 0.0
            if task.remaining_hours + hours.get(task.id, 0.0):
                res[task.id]['progress'] = round(min(
                    100.0 * hours.get(task.id, 0.0) / res[task.id][
                        'total_hours'], 99.99), 2)
    ...
    def _get_analytic_line(self, cr, uid, ids, context=None):
        result = []
        for aal in self.pool.get('account.analytic.line').browse(
    ...
    _columns = {
        'work_ids': fields.one2many('hr.analytic.timesheet', 'task_id',
                                    'Work done'),

        'effective_hours': fields.function(_progress_rate, multi="progress",
                                           method=True, string='Time Spent',
                                           help="Sum of spent hours of all "
        ...
        'delay_hours': fields.function(_progress_rate, multi="progress",
                                       method=True, string='Deduced Hours',
                                       help="Sum of spent hours with invoice"
                                            " factor of all tasks related to"
                                            " this project and its child"
                                            " projects.",
                                       store={'project.task': (
                                           lambda self, cr, uid, ids,
                                            c={}: ids,
                                           TASK_WATCHERS, 20),
                                           'account.analytic.line': (
                                               _get_analytic_line, AA_WATCHERS,
                                               20)}),

        'total_hours': fields.function(_progress_rate, multi="progress",
                                       method=True, string='Total Time',
                                       help="Sum of total hours of all"
                                            " tasks related to this project "
                                            "and its child projects.",
                                       store={'project.task': (
                                           lambda self, cr, uid, ids,
        ...
        'progress': fields.function(_progress_rate, multi="progress",
                                    method=True, string='Progress',
                                    type='float', group_operator="avg",
                                    help="Percent of tasks closed according "
                                         "to the total of tasks todo.",
                                    store={'project.task': (
...
class AccountAnalyticLine(osv.osv):
    ...
    def _compute_hours_with_factor(self, cr, uid, hours, factor_id,
                                   context=None):
    ...
    def _set_remaining_hours_create(self, cr, uid, vals, context=None):
        ...
        comp_hours = self._compute_hours_with_factor(cr, uid, hours, factor_id,
                                                     context)
        if comp_hours:
            cr.execute(
    ...
    def _set_remaining_hours_write(self, cr, uid, ids, vals, context=None):
        ...
        for line in self.browse(cr, uid, ids):
            ...
            new_task_id = vals.get('task_id',
                                   old_task_id)  # if no task_id in
            #  vals we assume it is equal to old
            ...
            if new_task_id:
                ...
                factor_id = vals.get('to_invoice',
                                     line.to_invoice and line.to_invoice.id
                                     or False)
                comp_hours = self._compute_hours_with_factor(cr, uid, hours,
                                                             factor_id,
                                                             context)
                old_factor = line.to_invoice and line.to_invoice.id or False
                old_comp_hours = self._compute_hours_with_factor(
                    cr, uid, line.unit_amount, old_factor, context)
                # we always execute request because invoice factor
                #  can be set to gratis
                cr.execute(
                    'update project_task set remaining_hours=remaining_hours'
                    ' - %s + (%s) where id=%s',
